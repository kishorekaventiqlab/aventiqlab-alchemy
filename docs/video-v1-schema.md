...
| `estimated_duration_minutes` | ✅ | Estimate. **alchemy-computed, not model-supplied** (since 2026-09-01): `generator.ts`'s `fillVideoEstimatedDuration` overwrites whatever the model wrote with `(sum of every beat's target_duration_sec, investigation CONTAINER beats excluded) / 60` — the exact arithmetic the duration-sum self-check itself performs, so that check can no longer fail from a model arithmetic slip (found live: the model correctly followed the prompt's "write beats first, then sum" instruction and still occasionally mis-added). Do NOT reintroduce a prompt-side "make the model get this right" fix on top of this — the value is already guaranteed correct by construction; a prompt change here would only affect what the model writes before it gets overwritten. The **authoritative** final duration remains `render.output.duration_sec` (§5.5), timed to the real voiceover. Vision QA flags if actual runtime is ~2x or ~0.5x this. |
